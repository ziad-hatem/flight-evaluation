...
export async function GET(req: Request) {
  try {
    ...
    if (!session || !session.user?.id) {
      console.log('Session validation failed:', { sessionExists: !!session, userIdExists: !!session?.user?.id });
...
export async function PUT(req: Request) {
  try {
    ...
    // Check if user is authenticated
    if (!session || !session.user?.id) {
      console.log('Session validation failed on update:', { sessionExists: !!session, userIdExists: !!session?.user?.id });
      return NextResponse.json(
        { error: 'Unauthorized' },
        { status: 401 }
      );
